...
def load_and_process_csv(csv_file):
    ...
    try:
        ...
        if len(df.columns) > 15:
            ...
            additional_columns = additional_columns.drop(columns=[col for col in additional_columns.columns if col.startswith("AUTOCORR")])

            # Drop Unnecessary Columns
            for column in DROPPED_COLUMNS:
                additional_columns = additional_columns.drop(column, axis=1)
